...
use crate::client::{BaseHTTPClient, HTTPConfig, LLMClient};
...
impl OpenAIClient {
    pub fn create_headers(config: &HTTPConfig) -> Result<HeaderMap, PotatoError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {}", config.token.clone()))
        ...
        if let Some(org) = config.organization.clone() {
            headers.insert(
                ...
                HeaderValue::from_str(&org).map_err(|e| PotatoError::Error(e.to_string()))?,
            );
        }
        ...
        if let Some(project) = config.project.clone() {
            headers.insert(
                ...
                HeaderValue::from_str(&project).map_err(|e| PotatoError::Error(e.to_string()))?,
            );
        }
